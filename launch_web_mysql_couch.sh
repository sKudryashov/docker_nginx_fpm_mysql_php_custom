...
HOST_XDEBUG_PORT=9002
CONTAINER_XDEBUG_PORT=9001

HOST_VOLUME=$WORKDIR
CONTAINER_VOLUME='/usr/share/nginx/html'

HOST_WEB_LOG=$HOST_NGINX_LOG
...
function create_dirs {
  ...
  if [ ! -d $HOST_NGINX_LOG ]; then
      mkdir -p $HOST_NGINX_LOG
  fi
...
function start_containers {
  ...
  docker run -d -p $HOST_XDEBUG_PORT:$CONTAINER_XDEBUG_PORT -p $HOST_PORT:$CONTAINER_PORT -p $HOST_SSH_PORT:$CONTAINER_SSH_PORT  \
